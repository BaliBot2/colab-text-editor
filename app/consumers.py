...
class EditorConsumer(AsyncWebsocketConsumer):
    async def connect(self):
        ...
        """
        self.doc_id = self.scope["url_route"]["kwargs"][
            "doc_id"
        ]  # Extract doc_id from URL
        self.room_group_name = (
            f"editor_{self.doc_id}"  # Create a unique group name per document
        )
        ...
        text_data = json.dumps({"content": document.state})
        await self.send(text_data)
        print("New client to message:", text_data)
    ...
    async def receive(self, text_data):
        ...
        try:
            ...
            await document.handle_changes(incoming_delta)
